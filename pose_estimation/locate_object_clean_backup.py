...
class LocateObject:
    ...
    def __init__(self, name=None):
        # custom parameters
        self.down_scale = 1  # down_scale speeds things up by already subsampling the image [power of 2]
        self.max_dist = 1.0  # crop points farther than this [meters]
        self.slop = 0.2  # max delay between rgb and depth image [seconds]
        self.debug_visualization = False  # visualize the process on an image topic
        ...
        self.im_width = 1280
        self.im_height = 720
        self.im_K = np.asarray([[906.523681640625, 0.0, 643.03515625], [0.0, 904.7564697265625, 360.9212341308594], [0.0, 0.0, 1.0]])

        self.ycbv_names_json = "/verefine/data/ycbv_names.json"
        self.ycbv_verefine = "/verefine/data/ycbv_verefine.json"
        ...
        meshes = [trimesh.load("/verefine/data/models/render/obj_{id:06}.ply".format(id = int(obj_id))).apply_transform(scale)
                       for obj_id in self.obj_ids]
        
        self.meshes_sampled = []
        for mesh, to_m, cloud_path in zip(meshes, object_to_m, cloud_paths):
            if os.path.exists(cloud_path):
                print(f"cloud_path: {cloud_path} already exists!")
        ...
        #scenes = ["pcd_001_standing_coated_clean.ply", "pcd_002_lying_coated_clean.ply", "pcd_003_standing_liquid_clean.ply", "pcd_004_lying_liquid_clean.ply", "pcd_005_standing_empty_clean.ply", "pcd_006_lying_empty_clean.ply"]
        scenes = ["pcd_006_lying_empty_clean.ply"]
        for scene in scenes:
            input_scene_pcd = o3d.io.read_point_cloud(f"/verefine/data/models/scenes/{scene}")
        
            # == preprocessing
            # -- get plane pose, inliers and scene points
            plane_pose, plane_pcd, scene_pcd, cloud_pcd, plane_indices = self.detector.detect(input_scene_pcd, self.max_dist)
            ...
            o3d.io.write_point_cloud(f"/verefine/data/models/scenes/results/plane_{scene}", plane_pcd)
            o3d.io.write_point_cloud(f"/verefine/data/models/scenes/results/object_{scene}", object_pcd)
            ...
            obj_models_to_search = ["101_canister_full_small", "102_LargeRinseFluidA_Bottle", "103_MediumRinseFluidK_Bottle", "104_SmallSoyBroth_Bottle"]
            ...
            for oi, object_hypotheses in enumerate(objects_hypotheses):
                ...
                for hi, object_hypothesis in enumerate(object_hypotheses):
                    ...
                    object_poses.append(object_hypothesis.transform)
                    object_types.append(object_hypothesis.model_id)
                    confidences.append(object_hypothesis.confidence)
 
            
            best_obj_poses = object_poses[::3]

            transformed_meshes = []
            icp_results = []
            for idx, mesh in enumerate(self.meshes_sampled):
                transf_mesh = copy.deepcopy(mesh)
                transf_mesh.transform(best_obj_poses[idx])
                o3d.io.write_point_cloud(f"/verefine/data/models/scenes/results/transf_{obj_models_to_search[idx]}_{scene}", transf_mesh)
                #transformed_meshes.append(transf_mesh)
                icp_registration = o3d.pipelines.registration.registration_icp(source=transf_mesh, target=input_scene_pcd, max_correspondence_distance=0.02)
                print(icp_registration.transformation)
                icp_mesh = copy.deepcopy(transf_mesh)
                icp_mesh.transform(icp_registration.transformation)
                o3d.io.write_point_cloud(f"/verefine/data/models/scenes/results/icp_{obj_models_to_search[idx]}_{scene}", icp_mesh)
        ...
        #transform -> init mesh to object_pose_hypot -> feed to icp
        #transform mesh to refined pose
