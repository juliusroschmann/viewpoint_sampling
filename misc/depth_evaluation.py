...
def main():
    ...
    title_list = []
    for obj in obj_list:
        title_list.append(f"{obj}_gt")
        title_list.append(f"{obj}_real")
        title_list.append(f"{obj}_diff")
    ...
    for scene in [scenes[0]]:
        ...
        folder_to_evaluate = f"/home/julius/Documents/Julius_03_x_auswertung/Julius_03_20/scenes/{scene}/depth_nerf"
        folder_to_save = os.path.dirname(os.path.dirname(os.path.dirname(folder_to_evaluate)))
        folder_to_save += f"/results_nerfacto_{scene}.csv"
        ...
        if len(canister_masks) != len(blender_depth) or len(small_bottle_masks) != len(blender_depth) or len(medium_bottle_masks) != len(blender_depth) or len(large_bottle_masks) != len(blender_depth) or len(to_eval_imgs) != len(blender_depth):
            ...
            mydict = {"File": [], "Scene": [], "Object": [], "Max": [],
                      "Min": [], "Median": [], "Mean": [], "Std": [], "Var": []}
            for idx, _ in enumerate(blender_depth):
                ...
                # canister_vals = calc_diff(canister_gt, canister_real)
                ...
                # mydict = append_to_dict(mydict, filename, scene, canister_vals, object_name=obj_list[0])
                mydict = append_to_dict(
                    mydict, filename, scene, small_bottle_vals, object_name=obj_list[1])
                mydict = append_to_dict(
                    mydict, filename, scene, medium_bottle_vals, object_name=obj_list[2])
                mydict = append_to_dict(
                    mydict, filename, scene, large_bottle_vals, object_name=obj_list[3])
                ...
                # cbar.set_ticks(np.arange(0, 100.5, 1))
                # # cbar.set_ticklabels(['low', 'medium', 'high'])
                # plt.show()
   
    df = pd.DataFrame.from_dict(data=mydict).to_csv(folder_to_save)
    print(f"Saving dataframe to: {folder_to_save}")
    #print(df.head(100))
    print(df["Max"].mean(skipna=True))
    print(df["Min"].mean(skipna=True))
    print(df["Median"].mean(skipna=True))
    print(df["Mean"].mean(skipna=True))
...
def calc_diff(just_diff, obj_gt, obj_real):
    ...
    if len(px_diff) == 0:
        ...
        return [np.max(px_diff), np.min(px_diff), np.median(px_diff), np.mean(px_diff), np.std(px_diff), np.var(px_diff)]


def append_to_dict(mydict, filename, scene, vals, object_name):
